...
def generate_tasks(
    ...
    if scene_type == "two_crane_crossing" and len(cranes) >= 2:
        crossing_partners = {
            cranes[0].crane_id: cranes[1],
            cranes[1].crane_id: cranes[0],
        }
    for crane in cranes:
        num_tasks = int(rng.integers(min_tasks, max_tasks + 1))
        for idx in range(num_tasks):
            if scene_type == "two_crane_crossing" and crane.crane_id in crossing_partners:
                other = crossing_partners[crane.crane_id]
